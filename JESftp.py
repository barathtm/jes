...
class JESftp:
    ...
    def connect(self, server=None, username=None, password=None):
        ...
        if len(conn_str) > 1:
            ...
            self.port = conn_str[1]  # Ensure port is an integer
        else:
            self.server = conn_str[0]
            self.port = 21
    ...
    def processJobOutput(self, infile, splitPages=False):
        ...
        if splitPages == True:
            ...
            for pg in pages:
                cnt += 1
                with open(f"{outfile_path_noprefix}-pg{cnt}.txt", 'w', encoding="utf-8") as out:
                    out.write(pg)

        else:
            with open(f"{outfile_path_noprefix}-pp.txt", 'w', encoding="utf-8") as out:
                for pg in pages:
                    out.write(pg)
    ...
    def processJob(self, infile, outfile=None):
        ...
        print(f"Waiting for completion of {JOBID}...")
        time.sleep(1)
    ...
    def createConfig(self, path):
        ...
        Currently only server credentials are implemented.
        ...
        server = input("Host [172.26.1.2]: ") 
        if server == "":
            server = '172.26.1.2'

        username = input("User ID [ibmuser]: ") 
        if username == "":
            username = 'ibmuser'

        password = getpass.getpass("Password [sandhata]: ")
        if password == "":
            password = 'sandhata'
